...
export default function NotificationsPage() {
  ...
  return <><PageHeader eyebrow="In-app source of truth" title="Notifications" /><StatusMessage>{message}</StatusMessage><section className="surface">{items.length === 0 ? <div className="empty"><p>No notifications yet.</p></div> : <ul className="item-list">{items.map((item) => <li key={item.id}><div><span className="eyebrow">{item.read_at ? "Read" : "New"}</span><h3>{item.title}</h3><p>{item.body}</p><p>{new Date(item.created_at).toLocaleString()}</p></div>{!item.read_at && <button onClick={() => void markRead(item)}>Mark read</button>}</li>)}</ul>}</section></>;
}
